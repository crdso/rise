  ...
  return (
    <Ctx.Provider value={{ push }}>
      ...
      <div className="fixed bottom-[88px] lg:bottom-4 right-4 left-4 lg:left-auto z-[60] flex flex-col gap-2 items-end pointer-events-none">
        <AnimatePresence>
          {toasts.map((t) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: 8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 4, scale: 0.98 }} className={`pointer-events-auto min-w-[280px] max-w-[360px] rounded-2xl border px-4 py-3 flex gap-3 shadow-[0_12px_32px_rgba(0,0,0,0.16)] ${t.variant === "error" ? "bg-red-50 border-red-200 text-red-800" : "bg-[var(--card)] border-[var(--border)] text-[var(--foreground)]"}`}>
              {t.variant === "error" ? <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" /> : <CheckCircle2 className="h-4 w-4 shrink-0 mt-0.5 text-emerald-500" />}
