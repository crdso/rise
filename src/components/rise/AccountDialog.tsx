...
export function AccountDialog({ open, onClose, onSave, initial }: { open: boolean; onClose: () => void; onSave: (data: { name: string; type: AccountType; color: string; initial_balance: number }) => Promise<void>; initial?: Account | null }) {
  ...
  const submit = async () => {
    ...
    setErr("");
    try {
      // só fecha depois do sucesso — o parent faz rethrow após o toast
      await onSave({ name: parsed.data.name, type: parsed.data.type, color: parsed.data.color || COLORS[0], initial_balance: parsed.data.initial_balance });
      onClose();
    } catch (e: unknown) {
        ...
        <>
          ...
          <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.98 }} className="fixed inset-x-0 bottom-0 lg:inset-auto lg:top-1/2 lg:left-1/2 lg:-translate-x-1/2 lg:-translate-y-1/2 z-50 w-full lg:max-w-[440px] rounded-t-[22px] lg:rounded-[20px] border border-[var(--border)] bg-[var(--elevated)] p-5 pb-[calc(1rem+var(--sab))] shadow-[0_28px_70px_rgba(0,0,0,0.55)]">
            ...
            <div className="mt-4 space-y-3">
              ...
              <Button onClick={submit} disabled={loading} aria-busy={loading} className="w-full rounded-full mt-2">{loading ? "Salvando..." : initial ? "Salvar" : "Criar conta"}</Button>
            </div>
          </motion.div>
        </>
      )}
