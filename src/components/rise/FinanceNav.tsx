  ...
  return (
    <div className="flex gap-1 p-1 rounded-full border border-[var(--border)] bg-[var(--card)] w-fit">
      {TABS.map(t=>{
        ...
        return <Link key={t.href} href={t.href} className={cn("px-4 py-1.5 rounded-full text-xs font-medium", active?"bg-[var(--accent)] text-[var(--accent-foreground)]":"text-[var(--muted-foreground)]")}>{t.label}</Link>;
      })}
    </div>
  );
}
